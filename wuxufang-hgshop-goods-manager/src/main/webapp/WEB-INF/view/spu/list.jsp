...
<div>
	<table class="table">
		...
		<c:forEach items="${pageInfo.list}" var="spu" varStatus="count">
			<tr>
				<td>${count.count+page.pageRecord}<input type="checkbox"
					value="${spu.id}" name="id">
				</td>
				<td>${spu.goodsName}</td>
				<td>${spu.caption}</td>
				<td><img src="/pic/${spu.smallPic}" width="50px" height="50px">
				</td>
				<td>
					...
					<button type="button" class="btn btn-info"
						onclick="addSku(${spu.id})">加sku</button>
				</td>
			</tr>
		</c:forEach>
		<tr>
